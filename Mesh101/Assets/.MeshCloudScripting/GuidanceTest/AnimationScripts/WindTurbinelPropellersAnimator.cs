...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class WindTurbinelPropellersAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, 1F, };

        protected WindTurbinelPropellersAnimator(in Guid ahandle, bool transfer)
        ...
        internal static WindTurbinelPropellersAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new WindTurbinelPropellersAnimator(handle, transfer: t));

            return result as WindTurbinelPropellersAnimator;
        }
    }
}